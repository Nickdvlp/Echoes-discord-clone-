...
const roleIconMap = {
  GUEST: null,
  MODERATER: <ShieldCheck className="w-4 h-4 ml-2 text-indigo-500" />,
...
const ChatItem = ({
  ...
  const isModerater = currentMember.role === MemberRole.MODERATER;
  const isOwner = currentMember.id === member.id;

  const canDeleteMessage = !deleted && (isAdmin || isModerater || isOwner);
        ...
        <div className="flex flex-col w-full">
          ...
          {!fileUrl && isEditing && (
            <Form {...form}>
              ...
              <span className="text-[15px] mt-1 text-zinc-400">
